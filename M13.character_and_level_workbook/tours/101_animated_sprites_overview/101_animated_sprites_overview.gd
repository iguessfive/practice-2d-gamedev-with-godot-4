...
func _build() -> void:
	...
	bubble_add_text([
		...
		"Notice how the frames play in sequence and loop. You can click the " + bbcode_generate_icon_image_by_name("Stop") + " [b]Stop[/b] button to pause the animation."
	])
	bubble_add_task_press_button(interface.spriteframes_frames_toolbar_play_button)
	complete_step()
