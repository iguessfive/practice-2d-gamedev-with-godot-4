...
func _build() -> void:
	...
	bubble_move_and_anchor(interface.canvas_item_editor, Bubble.At.CENTER_RIGHT)
	highlight_inspector_properties(["sprite_frames"])
	highlight_controls([interface.spriteframes], true)
	bubble_add_text([
	...
	highlight_filesystem_paths(["res://assets/lucy/run"])
	bubble_add_text([
	...
	bubble_add_text([
		...
		"[ul][b]Left click[/b] on the first frame (e.g., [b]run_000.png[/b])\n" + "Hold [b]Shift[/b] and [b]left click[/b] on the last frame (e.g., [b]run_005.png[/b])[/ul]",
		"",
	...
	bubble_set_avatar_at(Bubble.AvatarAt.CENTER)
	bubble_move_and_anchor(interface.canvas_item_editor, Bubble.At.BOTTOM_CENTER)
	highlight_filesystem_paths(run_frames)
	...
	bubble_add_task_toggle_button(
		...
		true,
		"Ensure looping is enabled for the animation."
	)
	complete_step()
