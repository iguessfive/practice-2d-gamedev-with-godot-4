...
func _build() -> void:
	...
	bubble_add_task_press_button(interface.spriteframes_animation_toolbar_add_animation_button, "New Animation")
	complete_step()
	...
	bubble_add_text([
		...
		"In the [b]FileSystem[/b] dock, find and select the idle animation frames ([b]idle.png[/b] and [b]blinking.png[/b]). You can press the [b]Crtl[/b] key ([b]Cmd[/b] on macOS) before clicking to add a file to the selection."
	])
	bubble_add_task(
	...
	bubble_set_avatar_at(Bubble.AvatarAt.LEFT)
	highlight_filesystem_paths(["res://assets/lucy/idle/idle.png", "res://assets/lucy/idle/blinking.png"], true)
	highlight_controls([interface.spriteframes_frames_list], true)
	bubble_add_text([
		"Drag and drop the selected idle frames into the [b]Frames View[/b] of the SpriteFrames editor.",
